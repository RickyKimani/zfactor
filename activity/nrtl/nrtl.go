...
// NRTL contains the information required to evaluate
// activity coefficients using the NRTL model.
type NRTL struct {
...
// TauCoefficients contains the coefficients used to
// evaluate temperature-dependent NRTL interaction
// parameters.
type TauCoefficients struct {
	A [][]float64
	B [][]float64
	C [][]float64
	D [][]float64
...
// Tau evaluates the temperature-dependent binary interaction
// parameters using
//
//	τij = aij + bij/T + cij ln(T) + dijT
//
// where T is the absolute temperature in kelvin.
func (tc TauCoefficients) InteractionParameters(T float64) ([][]float64, error) {
	if T <= 0 {
...
func (n NRTL) BinaryInfiniteDilution() ([]float64, error) {
	...
	g12 := math.Exp(-n.Alpha[0][1] * n.Tau[0][1])
	g21 := math.Exp(-n.Alpha[1][0] * n.Tau[1][0])

	lnGamma1 := n.Tau[1][0] +
		g12*(n.Tau[0][1]-n.Tau[1][0])

	lnGamma2 := n.Tau[0][1] +
		g21*(n.Tau[1][0]-n.Tau[0][1])
